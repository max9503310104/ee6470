...
Testbench::~Testbench() {
	...
	cout << "Simulation time = " << total_run_time << endl;
	//cout << ", cycle count = " << (int)(total_run_time.to_double() * 1e-3 / 10)<< endl;
}
...
void Testbench::fetch_result() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = total & 0x0000ff;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = (total & 0x00ff00) >> 8;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = (total & 0xff0000) >> 16;
