...
void Testbench::do_test() {
	...
	for (row = 0; row < height + 1; row++) {
		for (col = 0; col < width; col++) {
			if (row < height) {
				R = *(source_bitmap + bytes_per_pixel * (width * row + col) + 2);
				G = *(source_bitmap + bytes_per_pixel * (width * row + col) + 1);
				B = *(source_bitmap + bytes_per_pixel * (width * row + col) + 0);
				data.uc[0] = R;
				data.uc[1] = G;
				data.uc[2] = B;
			} else {
...
void Testbench::receive_output()
{
	...
	while(1) {
    	...
    	R_i = data.uc[0];
      	G_i = data.uc[1];
      	B_i = data.uc[2];	
		*(target_bitmap + bytes_per_pixel * (width * row + col) + 2) = R_i;
		*(target_bitmap + bytes_per_pixel * (width * row + col) + 1) = G_i;
		*(target_bitmap + bytes_per_pixel * (width * row + col) + 0) = B_i;
		//cout << "tb read r c rgb " << row << col <<" "<<R_i<<G_i<<B_i<<endl;
