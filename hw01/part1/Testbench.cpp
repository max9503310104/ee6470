...
void Testbench::do_filter() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = r_flt;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = g_flt;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = b_flt;
    }
  }
  sc_stop();
}
