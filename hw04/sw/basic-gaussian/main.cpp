...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){
      ...
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 2) = result[0];
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 1) = result[1];
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 0) = result[2];
    }
  }
