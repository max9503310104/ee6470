...
public:
	...
	addr_t gaussSeidel_start_addr = 0x73000000;
 	addr_t gaussSeidel_size = 0x01000000;
 	addr_t gaussSeidel_end_addr = gaussSeidel_start_addr + gaussSeidel_size - 1;
	addr_t gaussSeidel1_start_addr = 0x74000000;
 	addr_t gaussSeidel1_size = 0x01000000;
 	addr_t gaussSeidel1_end_addr = gaussSeidel1_start_addr + gaussSeidel1_size - 1;
	addr_t gaussSeidel2_start_addr = 0x75000000;
 	addr_t gaussSeidel2_size = 0x01000000;
 	addr_t gaussSeidel2_end_addr = gaussSeidel2_start_addr + gaussSeidel2_size - 1;
	addr_t gaussSeidel3_start_addr = 0x76000000;
 	addr_t gaussSeidel3_size = 0x01000000;
 	addr_t gaussSeidel3_end_addr = gaussSeidel3_start_addr + gaussSeidel3_size - 1;
	addr_t gaussSeidel4_start_addr = 0x77000000;
 	addr_t gaussSeidel4_size = 0x01000000;
 	addr_t gaussSeidel4_end_addr = gaussSeidel4_start_addr + gaussSeidel4_size - 1;
	addr_t gaussSeidel5_start_addr = 0x78000000;
 	addr_t gaussSeidel5_size = 0x01000000;
 	addr_t gaussSeidel5_end_addr = gaussSeidel5_start_addr + gaussSeidel5_size - 1;
	addr_t gaussSeidel6_start_addr = 0x79000000;
 	addr_t gaussSeidel6_size = 0x01000000;
 	addr_t gaussSeidel6_end_addr = gaussSeidel6_start_addr + gaussSeidel6_size - 1;
	addr_t gaussSeidel7_start_addr = 0x7a000000;
 	addr_t gaussSeidel7_size = 0x01000000;
 	addr_t gaussSeidel7_end_addr = gaussSeidel7_start_addr + gaussSeidel7_size - 1;
...
int sc_main(int argc, char **argv) {
	...
	GaussSeidel gaussSeidel("gauss_seidel");
	GaussSeidel gaussSeidel1("gauss_seidel1");
	GaussSeidel gaussSeidel2("gauss_seidel2");
	GaussSeidel gaussSeidel3("gauss_seidel3");
	GaussSeidel gaussSeidel4("gauss_seidel4");
	GaussSeidel gaussSeidel5("gauss_seidel5");
	GaussSeidel gaussSeidel6("gauss_seidel6");
	GaussSeidel gaussSeidel7("gauss_seidel7");

	MemoryDMI dmi = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
	InstrMemoryProxy instr0_mem(dmi, core0);
	InstrMemoryProxy instr1_mem(dmi, core1);
	...
	if (opt.use_instr_dmi) {
		instr0_mem_if = &instr0_mem;
		instr1_mem_if = &instr1_mem;
	}
	if (opt.use_data_dmi) {
	...
	core0.init(instr0_mem_if, data0_mem_if, &clint, entry_point, rv32_align_address(opt.mem_end_addr) - 3);
	core1.init(instr1_mem_if, data1_mem_if, &clint, entry_point, rv32_align_address(opt.mem_end_addr) - 2097151);
	//core0.init(instr0_mem_if, data0_mem_if, &clint, loader.get_entrypoint(), opt.mem_end_addr - 3);
	//core1.init(instr1_mem_if, data1_mem_if, &clint, loader.get_entrypoint(), opt.mem_end_addr - 16777215);
	...
	bus.ports[12] = new PortMapping(opt.gaussSeidel_start_addr, opt.gaussSeidel_end_addr);
	bus.ports[13] = new PortMapping(opt.gaussSeidel1_start_addr, opt.gaussSeidel1_end_addr);
	bus.ports[14] = new PortMapping(opt.gaussSeidel2_start_addr, opt.gaussSeidel2_end_addr);
	bus.ports[15] = new PortMapping(opt.gaussSeidel3_start_addr, opt.gaussSeidel3_end_addr);
	bus.ports[16] = new PortMapping(opt.gaussSeidel4_start_addr, opt.gaussSeidel4_end_addr);
	bus.ports[17] = new PortMapping(opt.gaussSeidel5_start_addr, opt.gaussSeidel5_end_addr);
	bus.ports[18] = new PortMapping(opt.gaussSeidel6_start_addr, opt.gaussSeidel6_end_addr);
	bus.ports[19] = new PortMapping(opt.gaussSeidel7_start_addr, opt.gaussSeidel7_end_addr);
	...
	bus.isocks[12].bind(gaussSeidel.tsock);
	bus.isocks[13].bind(gaussSeidel1.tsock);
	bus.isocks[14].bind(gaussSeidel2.tsock);
	bus.isocks[15].bind(gaussSeidel3.tsock);
	bus.isocks[16].bind(gaussSeidel4.tsock);
	bus.isocks[17].bind(gaussSeidel5.tsock);
	bus.isocks[18].bind(gaussSeidel6.tsock);
	bus.isocks[19].bind(gaussSeidel7.tsock);

	// connect interrupt signals/communication
	plic.target_harts[0] = &core0;
	plic.target_harts[3] = &core1;
	clint.target_harts[0] = &core0;
	clint.target_harts[3] = &core1;
	sensor.plic = &plic;
	dma.plic = &plic;
	timer.plic = &plic;
	sensor2.plic = &plic;
	...
	if (opt.use_debug_runner) {
		auto server0 = new GDBServer("GDBServer", threads, &dbg_if, opt.debug_port);
		auto server1 = new GDBServer("GDBServer", threads, &dbg_if, opt.debug_port);
